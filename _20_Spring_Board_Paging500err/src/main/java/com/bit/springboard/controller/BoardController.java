...
//    @GetMapping("free-list.do") Post / Get 방식의 요청 모두 처리 하도록
    ...
    public String freeListView(Model model, @RequestParam Map<String, String> searchMap, Criteria criteria) {
        ...
        model.addAttribute("freeBoardList", boardService.getBoardList(searchMap, criteria));
        model.addAttribute("searchMap", searchMap);

        // 게시글의 총 개수
        int total = boardService.getBoardTotalCnt(searchMap);

        // 화면에서 페이지를 표시하기위해 PageDto 객체 화면에 전달
        model.addAttribute("page", new PageDto(criteria, total));
...
//    @GetMapping("/notice-list.do") Get Post 모두 처리 가능한 RequestMapping 으로 변경!
    ...
    public String noticeListView(Model model, @RequestParam Map<String, String> searchMap, Criteria criteria) {
        boardService = applicationContext.getBean("noticeServiceImpl", BoardService.class);

        criteria.setAmount(9);

        // boardService.getBoardList() 의 이름을 noticeList 로 설정해서 모델객체에 넣어줌.
        model.addAttribute("noticeList", boardService.getBoardList(searchMap, criteria));
        // notice-list.jsp 에서 jstl 을 이용한 forEach 문의 items 로 꺼낼 수 있음.
        model.addAttribute("searchMap", searchMap);
        // model 에 속성을 여러 개 넣을 수 있다.

        int total = boardService.getBoardTotalCnt(searchMap);
        model.addAttribute("page", new NoticePageDto(criteria, total));
